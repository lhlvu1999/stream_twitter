...
class UserRepository:
    ...
    def get_user_post_tweet(self, tweet_id: str):
        return self.session.query(models.User).join(models.Fact_tweet).filter(
...
class TweetRepository:
    ...
    def get_tweets(self, skip: int = 0, limit: int = 100):
        return self.session.query(models.Fact_tweet).offset(skip).limit(limit).all()

    def get_tweets_with_subject(self, subject: str, skip: int = 0, limit: int = 100):
        return self.session.query(models.Fact_tweet).join(models.Subject).filter(
    ...
    def get_sentiment(self, skip: int = 0, limit: int = 100):
        return self.session.query(models.Fact_tweet.sentiment).offset(skip).limit(limit).all()

    def get_text(self, skip: int = 0, limit: int = 100):
        return self.session.query(models.Fact_tweet.tweet_id, models.Fact_tweet.text).offset(skip).limit(limit).all()
    ...
    def insert_fact_tweet(self, tweet_id, user_id, subject_id, date_tweet, text, source, lang):
        ...
        self.session.add(session_fact_tweet)
        self.session.commit()
        return session_fact_tweet

    def update_sentiment(self, tweet_id: str, sentiment: int):
        for c in self.session.query(models.Fact_tweet).all():
...
class SubjectRepository:
    ...
    def get_subject(self, subject: str):
        return self.session.query(models.Subject.name).filter(models.Subject.name == subject).first()

    def get_subject_id(self, subject: str):
        return self.session.query(models.Subject.id).filter(models.Subject.name == subject).first()

    def get_subject_with_keyword(self, keyword: str):
        return self.session.query(models.Subject.name).join(models.Keyword).filter(
    ...
    def insert_subject(self, subject: str):
        ...
        self.session.add(session_subject)
        self.session.commit()
...
class KeywordRepository:
    ...
    def get_keyword_with_subject(self, subject: str):
        return self.session.query(models.Keyword.name).join(models.Subject).filter(
            models.Subject.id == models.Keyword.subject_id,
...
class DateRepository:
    ...
    def get_date(self, date_id: str):
        return self.session.query(models.Date).filter(models.Date.id == date_id).first()
